package com.javaedge.seckill.service;
...
import com.javaedge.commons.model.pojo.VoucherOrders;
...
import com.javaedge.seckill.mapper.SeckillVouchersMapper;
import com.javaedge.seckill.mapper.VoucherOrdersMapper;
import com.javaedge.seckill.model.RedisLock;
...
public class SeckillService {
    ...
    public ResultInfo doSeckill(Integer voucherId, String accessToken, String path) {
        // 基本参数校验
        AssertUtil.isTrue(voucherId == null || voucherId < 0, "请选择需要秒杀的代金券");
        AssertUtil.isNotEmpty(accessToken, "请登录");

        // 注释原始的 关系型数据库 的流程
        // 判断此代金券是否加入秒杀
//         SeckillVouchers seckillVouchers = seckillVouchersMapper.selectVoucher(voucherId);
//         AssertUtil.isTrue(seckillVouchers == null, "该代金券并未有秒杀活动");
        // 判断是否有效
//         AssertUtil.isTrue(seckillVouchers.getIsValid() == 0, "该活动已结束");

        // 采用 Redis
        String key = RedisKeyConstant.seckill_vouchers.getKey() + voucherId;
        Map<String, Object> vouchersMap = redisTemplate.opsForHash().entries(key);
        SeckillVouchers seckillVouchers = BeanUtil.mapToBean(vouchersMap, SeckillVouchers.class, true, null);

        // 判断是否开始、结束
        Date now = new Date();
        AssertUtil.isTrue(now.before(seckillVouchers.getStartTime()), "该秒杀还未开始");
//        AssertUtil.isTrue(now.after(seckillVouchers.getEndTime()), "该秒杀已结束");
        // 判断是否卖完
        ...
        try {
            ...
            if (isLocked) {
                ...
                AssertUtil.isTrue(count == 0, "用户秒杀失败");
            ...
            if (e instanceof ParameterException) {
                return ResultInfoUtil.buildError(0, "该券已售完", path);
            }
        }

        return ResultInfoUtil.buildSuccess(path, "秒杀成功");
    }
    ...
    public void addSeckillVouchers(SeckillVouchers seckillVouchers) {
        // 非空校验
        AssertUtil.isTrue(seckillVouchers.getFkVoucherId() == null, "请选择需要秒杀的代金券");
        AssertUtil.isTrue(seckillVouchers.getAmount() == 0, "请输入秒杀总数量");
        Date now = new Date();
        ...
        AssertUtil.isTrue(now.before(seckillVouchers.getEndTime()), "结束时间不能早于当前时间");
        ...
        // AssertUtil.isTrue(seckillVouchersFromDb != null, "该券已经拥有了秒杀活动");
        // 插入数据库
        ...
        // 如果使用 string 需要序列化传输再反序列化解析比较麻烦，而使用 hash 可直接得到目标字段,而
        // hash在 redis 中不会被序列化反序列化，此时的性能较好
        Map<String, Object> vouchersMap = redisTemplate.opsForHash().entries(key);
        AssertUtil.isTrue(!vouchersMap.isEmpty() && (int) vouchersMap.get("amount") > 0, "该券已拥有秒杀活动");
